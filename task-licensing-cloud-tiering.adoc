...
您過去將內部部署的各項資料分層保存至雲端時、可能FabricPool 使用的*《*環*》授權ONTAP 僅適用於ONTAP 無法存取網際網路的站台（又稱為「暗站」）中的非執行部署。如果您部署的是深度站台、您將FabricPool 會使用System Manager或ONTAP CLI在每個叢集上安裝一份「版本資訊」授權。
